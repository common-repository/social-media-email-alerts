...
Text Domain: social-media-email-alerts
*/
...
/*****************************/
	...
	function init() {
		...
		$this->install();
		define('WPSMA_ref', isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '');
		define('WPSMA_query', isset($_SERVER['QUERY_STRING']) ? $_SERVER['QUERY_STRING'] : '');
		define('WPSMA_uri', isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '');
	...
	function js_init(){
		...
		if (isset($_POST['wpsma_action']) && $_POST['wpsma_action'] == 1) {
			define('WPSMA_ref', isset($_POST['wpsma_ref']) ? urldecode($_POST['wpsma_ref']) : '');
			define('WPSMA_query', isset($_SERVER['QUERY_STRING']) ? $_SERVER['QUERY_STRING'] : '');
			define('WPSMA_uri', isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '');
	...
	function get_domain($ref){
		...
		if (isset($parts['host'])) {
			...
			if( isset( $wra_sites[$domain] ) && is_array( $wra_sites[$domain] ) ) {
				return $domain;
			}
		}
	...
	function get_query($qs){
		...
		if ($qs) {
			...
			foreach($query as $query_string){
				if( isset( $wra_sites[$query_string] ) && is_array($wra_sites[$query_string])){
					return $query_string;
				}
			}
		}
	...
	function update_sites($options) {
		...
		foreach($options as $option) {
			...
			if($option['site'] && $option['min_visits']){
				...
				$sites[$option['site']] =
					array( 	'url' => isset($option['url']) ? $option['url'] : '',
...
/*****************************/
	...
	function menu() {
		add_options_page('Social Media Email Alerts', 'Social Media Email Alerts', 'manage_options', __FILE__,array($this,'admin'));
	}
...
/*****************************/
	...
	function admin(){
		...
		if(isset($_GET['remove']) && $_GET['remove'] && isset($_GET['to_remove']) && $_GET['to_remove'])
					...
					<table cellpadding="0" cellspacing="0" id="site_links">
						...
						if($sites){
							...
							while (list($name, $ops) = each($sites)) {	
								$name = esc_attr(stripslashes($name));
								echo "
									<tr>
			...
			<table id="site_links" border="1" rules="rows" frame="hsides">
				...
				if(is_array($groups)){
					...
					foreach($groups as $group){
						...
						?>
								<tr><td valign="top"><?php echo date('F j, Y h:i:s A', $hit['time']);?></td><td valign="top"><?php echo $hit['match'];?></td><td valign="top" align="center"><?php echo $total;?></td><td valign="top"><?php echo $referrer_list;?></td><td valign="top"><a href="<?php echo $hit_page_url;?>"><?php echo $hit_page;?></a></td><td valign="top" align="center"><a href="./options-general.php?page=social-media-email-alerts/social-media-email-alerts.php&amp;remove=1&amp;to_remove=<?php echo $hit['id']?>">X</a></td></tr>
						<?php
